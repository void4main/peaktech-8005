...
fn list_available_ports() {
    ...
    for p in ports {
        println!("Port {} is available.", p.port_name);
    }
}
...
fn main() {
    ...
    if !(is_port_available(port_peaktech.to_string())) {
        list_available_ports();
        panic!("Port '{port_peaktech}' not available.");
    }
    ...
    loop {
        ...
        if serial_buf[0] == PT8005_INDICATOR_DATA {
            ...
            port.read(serial_buf_b1.as_mut_slice())
                .expect("Found no data!");
            let mut serial_buf_b2: Vec<u8> = vec![0; 1];
            port.read(serial_buf_b2.as_mut_slice())
                .expect("Found no data!");
            // byte1 to float
